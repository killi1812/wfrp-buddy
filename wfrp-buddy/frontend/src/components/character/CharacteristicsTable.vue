...
const labels: Record<string, string> = {  WeaponSkill: 'WS', BalisticSkill: 'BS', Strength: 'S', Toughnes: 'T', Initative: 'I',
      ...
      <tbody>
        <tr>
          ...
          <td v-for="key in characteristicKeys" :key="key" class="pa-1">
            <v-number-input v-model="store.char.Characteristic[key].Basic" hide-details density="compact"
        ...
        <tr>
          ...
          <td v-for="key in characteristicKeys" :key="key" class="pa-1">
            <v-number-input v-model="store.char.Characteristic[key].Advances" hide-details density="compact"
              variant="plain" class="centered-input" control-variant="stacked" hide-controls />
          </td>
        </tr>
        <tr class="current-row">
          ...
          <td v-for="key in characteristicKeys" :key="key"
            ...
            {{ store.char.Characteristic[key].Basic + store.char.Characteristic[key].Advances }}
          </td>
        </tr>
        <tr class="bonus-row">
          <td class="row-label">Bonus</td>
          <td v-for="key in characteristicKeys" :key="key" class="text-center text-h6 py-1">
            {{ Math.floor((store.char.Characteristic[key].Basic + store.char.Characteristic[key].Advances) / 10) }}
          </td>
        </tr>
      </tbody>
    </v-table>
